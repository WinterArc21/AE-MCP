...
export async function setWorkAreaHelper(params: { compId: number; start: number; duration: number }) {
  const { compId, start, duration } = params;
  const script = wrapWithReturn(
...
export async function setCompRendererHelper(params: { compId: number; renderer: string }) {
  const { compId, renderer } = params;
...
export async function setMotionBlurSettingsHelper(params: {
  compId: number;
  enabled?: boolean | undefined;
  shutterAngle?: number | undefined;
  shutterPhase?: number | undefined;
  samplesPerFrame?: number | undefined;
  adaptiveSampleLimit?: number | undefined;
}) {
  const { compId, enabled, shutterAngle, shutterPhase, samplesPerFrame, adaptiveSampleLimit } = params;
  let setLines = "";
